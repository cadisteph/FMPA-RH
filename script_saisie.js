...
function genererAvancementSocle(agent) {
    ...
    const itemsHtml = socleFormations.map(f => {
        ...
        return `<span class="fma-item"><span style="background-color:#0284c7; color: #ffffff; font-weight:600;">${escapeHtml(f.libelle)} :</span> <span class="${styleClass}">${fait}/${quotaRequis}h</span></span>`;
    }).filter(Boolean);
...
function genererAvancementSpecialites(agent) {
    ...
    const itemsHtml = formationsSpec.map(f => {
        ...
        return `<span class="fma-item"><span style="background-color:#8b5cf6; color: #ffffff; font-weight:600;">${escapeHtml(f.libelle)} :</span> <span class="${styleClass}">${fait}/${quotaRequis}h</span></span>`;
    }).filter(Boolean);
