...
async function ouvrirTableauAttribution() {
    ...
    if (!window.showOpenFilePicker) {
        alert(
            "❌ Votre navigateur ne prend pas en charge l'ouverture de fichiers.\n\n" +
            "Utilisez Microsoft Edge ou Google Chrome."
        );
        return;
    }
    ...
    try {
        // Ouvre l'Explorateur Windows ciblé sur les fichiers Excel
        const [handle] = await window.showOpenFilePicker({
            types: [{
                description: "Tableau d'attribution Excel",
                accept: { "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet": [".xlsx"] }
            }],
            multiple: false
        });
        ...
        const classeurAttribution = XLSX.read(buffer, { type: "array", cellDates: true });

        console.log("Fichier chargé :", file.name);
        console.log("Onglets disponibles :", classeurAttribution.SheetNames);

        alert(`✅ Fichier "${file.name}" ouvert avec succès !\n\n` +
              `Onglets trouvés (${classeurAttribution.SheetNames.length}) :\n- ` + 
              classeurAttribution.SheetNames.join("\n- "));

        // Tu peux stocker le handle ou le classeur si tu souhaites traiter les données plus tard :
        window.classeurAttribution = classeurAttribution;

    } catch (err) {
        // Si l'utilisateur annule la sélection (ferme la fenêtre)
        if (err && err.name === "AbortError") return;

        console.error("Erreur lors de l'ouverture du Tableau d'attribution :", err);
