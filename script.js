const CODE_ACCES_RH = "1811";
    ...
    function changerOnglet(ongletId, bouton) {
        ...
        if ((ongletId === 'onglet-rh' || ongletId === 'onglet-config') && !estAuthentifieRH) {
            ...
            document.getElementById("inputCodeSecret").focus();
            return;
        }
    ...
    function mettreAJourAffichageAge() {
        const dateStr = document.getElementById("agentNaissanceDate").value;
        const label = document.getElementById("labelAge");
        if (label) label.innerText = calculerAge(dateStr);
    }
    ...
    function editerAgent(id) {
        ...
        document.getElementById("btnSupprimerAgent").style.display = "inline-flex";
        document.getElementById("btnRazFmaAgent").style.display = "inline-flex";
    ...
    function viderFormulaireRH() {
        ...
        document.getElementById("btnRazFmaAgent").style.display = "none";
    ...
    function actualiserTableauSuiviSpe() {
        ...
        document.getElementById("compteurAgentsSuiviSpe").innerText = `${agents.length} spécialiste(s)`;
    ...
    function mettreAJourCompteurFMA() {
        ...
        document.getElementById("fmaInfoSelection").innerHTML = `👥 <strong>${nb} agent(s) sélectionné(s)</strong>`;
    }
    ...
    function mettreAJourCompteurSpe() {
        ...
        document.getElementById("speInfoSelection").innerHTML = `👥 <strong>${nb} spécialiste(s) sélectionné(s)</strong>`;
    }
    ...
    function calculerDureeSession() {
        ...
        if (hDeb && hFin) {
            ...
            document.getElementById("fmaDureeCalculee").innerText = (diff / 60).toFixed(1) + " h";
        }
    }
    ...
    function calculerDureeSessionSpe() {
        ...
        if (hDeb && hFin) {
            ...
            let diff = (h2 * 60 + m2) - (h1 * 60 + m1);
            if (diff < 0) diff += 24 * 60;
            document.getElementById("speDureeCalculee").innerText = (diff / 60).toFixed(1) + " h";
        }
    }
    ...
    function editerModuleCatalogue(key) {
        ...
        document.getElementById("titreFormModule").innerText = "Modifier le Module";
        document.getElementById("btnEnregistrerModule").innerText = "Mettre à jour";
        document.getElementById("btnAnnulerEditModule").style.display = "inline-flex";
    }

    function viderFormulaireCatalogue() {
        document.getElementById("formCatalogueModule").reset();
        document.getElementById("editModuleKey").value = "";
        document.getElementById("titreFormModule").innerText = "Ajouter / Modifier Module";
        document.getElementById("btnEnregistrerModule").innerText = "Ajouter au catalogue";
        document.getElementById("btnAnnulerEditModule").style.display = "none";
    }
...
// 1. Ajouter un agent dans la liste en mémoire
function ajouterAgent(nouvelAgent) {
  listeAgents.push(nouvelAgent);
  
  // Met à jour le tableau à l'écran
  if (typeof afficherAgents === "function") {
    afficherAgents(listeAgents);
  }
}

// 2. Supprimer un agent de la liste en mémoire par son index
